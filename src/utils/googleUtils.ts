...
export const getGoogleUser = async ({
  access_token,
}: GoogleToken): Promise<GoogleUserResult> => {
  try {
...
export const userLogOrCreate = async (googleUser: GoogleUser): Promise<UserResponseWithToken> => {
  try {
    ...
    if (user) {
      ...
      if (googleUser?.invitation) {
        const decoded = jwt.verify(
          googleUser.invitation,
          JWT_SECRET
        ) as JwtPayload & {
          story: string;
          email: string;
        ...
        if (googleUser.email !== email) throw new Error("Invalid invite token");
        ...
        if (
          ...
          const updatedInvitationCodes = storyInvited.invitationCode.filter(
            (token:any) => token !== googleUser.invitation
          );
